import sorteoHeroBg from '@/assets/sorteo-hero.webp';
        ...
        <PublicLayout>
            ...
            >
                <div
                    ...
                    style={{
                        backgroundImage: `url(${sorteoHeroBg})`,
                        ...
                        {PASOS.map((paso, i) => (
                            <div key={paso.n} className="flex flex-1 items-start">
                                <div className="flex flex-col items-center gap-1.5">
                                    <div className="flex size-7 items-center justify-center rounded-full bg-gold text-xs font-bold text-bg">
                                        {paso.n}
                                    </div>
                                ...
                                {i < PASOS.length - 1 && (
                                    <div className="mt-3.5 flex-1 border-t-2 border-dashed border-gold/20" />
                                )}
                            </div>
                        ))}
                    </div>
...
function RegistroForm({ sorteoId, terminos, onSuccess }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        website: '', // honeypot — los humanos nunca lo llenan
    });

    function submit(e) {
        e.preventDefault();
        post(`/sorteos/${sorteoId}/registrar`, { forceFormData: true, onSuccess: () => onSuccess() });
    }
    ...
    function onWhatsappChange(e) {
        setData('whatsapp', e.target.value.replace(/\D/g, '').slice(0, 9));
    }
    ...
    return (
        <>
            ...
            <form onSubmit={submit} noValidate className="space-y-5 border border-gold/20 bg-surface p-5">
                {/* Honeypot: oculto con CSS, nunca con type="hidden" para que los bots lo vean */}
                ...
                <Field label="WhatsApp" error={errors.whatsapp}>
                    <input
                        type="text" inputMode="numeric" value={data.whatsapp}
                        onChange={onWhatsappChange} maxLength={9} pattern="\d{9}"
                        className={inputCls(errors.whatsapp)} placeholder="999999999"
                    />
                </Field>

                {/* Upload con instrucciones claras */}
                <Field label="Foto del comprobante" error={errors.comprobante}>
                    <div
                        className={[
                            'cursor-pointer overflow-hidden border-2 border-dashed bg-surface2 transition-all duration-150',
                            dragOver ? 'border-gold bg-gold/5 scale-[1.01]' :
